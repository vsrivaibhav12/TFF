...
export default function NewTaskDialog({ clients, team, defaultClientId, triggerLabel = 'New task', triggerVariant = 'default', mode = 'admin' }: Props) {
  ...
  function save() {
    ...
    startTransition(async () => {
      ...
      if (r.success) {
        ...
        setOpen(false);
        setF({ client_id: defaultClientId ?? '', sub_service_id: '', task_template_id: '', title: '', description: '', priority: 'medium', assigned_to: '', due_date: '', period_year: '', period_month: '', period_quarter: '', arn_reference: '', is_arn_client_visible: false });
      } else {
        toast.error(r.error);
      }
    });
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="space-y-3" onKeyDown={(e) => { if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') { e.preventDefault(); save(); } }}>
          ...
          <div className="space-y-2">
            <Label>Title *</Label>
            <Input value={f.title} onChange={(e) => set('title', e.target.value)} placeholder="e.g. GSTR-3B — Acme Pvt Ltd" data-testid="task-title" />
            <p className="text-xs text-zinc-500">Auto-filled from sub-service + client. Edit if needed.</p>
          </div>
          <div className="space-y-2">
