...
import PriorPeriodsPanel from './prior-periods-panel';
...
export default function BizlensStudioInputForm({ report, clientId, role, snapshots = [] }: Props) {
  ...
  const handlePublish = async () => {
    setPublishing(true);
    try {
      const res = await publishBizlensReport(report.id as string, clientId);
      if (res && (res as any).success === false) {
        toast.error((res as any).error ?? 'Failed to publish');
        return;
      }
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-24">
      ...
      <Tabs defaultValue="pl" className="w-full">
        <TabsList className="w-full justify-start p-1 bg-zinc-100/50 rounded-2xl overflow-x-auto h-auto">
          ...
          <TabsTrigger value="prior" className="rounded-xl px-6 py-3 font-bold text-base data-[state=active]:bg-white data-[state=active]:shadow-sm">Prior periods</TabsTrigger>
        </TabsList>

        <TabsContent value="pl" className="space-y-6 mt-6">
          <Card className="rounded-xl border border-zinc-200 bg-white overflow-hidden">
          ...
          <Card className="rounded-xl border border-zinc-200 bg-white overflow-hidden">
            ...
            <CardContent className="p-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="col-span-full pt-4 border-t border-zinc-100 mt-4">
        ...
        <TabsContent value="bs" className="space-y-6 mt-6">
          <Card className="rounded-xl border border-zinc-200 bg-white overflow-hidden">
            ...
            <CardContent className="p-5 grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <TabsContent value="strategic" className="space-y-6 mt-6">
          <Card className="rounded-xl border border-zinc-200 bg-white overflow-hidden">
            ...
            <CardContent className="p-5 grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <TabsContent value="prior" className="space-y-6 mt-6">
          <PriorPeriodsPanel clientId={clientId} snapshots={snapshots} />
        </TabsContent>
