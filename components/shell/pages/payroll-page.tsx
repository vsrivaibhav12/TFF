...
export default async function PayrollPage({ rolePrefix }: Props) {
  ...
  const exportData = (runs ?? []).map((r: any) => ({
    member: r.users_profile?.full_name ?? '',
    period: `${r.month}/${r.year}`,
    present_days: r.actual_present_days ?? 0,
    leave_days: r.actual_leave_days ?? 0,
    gross: r.gross_salary ?? 0,
    deductions: r.total_deductions ?? 0,
    net: r.final_salary ?? 0,
    status: r.status,
  }));
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Payroll"
        subtitle="Monthly payroll runs computed from attendance + per-user settings."
        actions={
          <>
            <ExportButton data={exportData} filename="payroll-export" format="csv" />
          ...
          <div className="overflow-x-auto">
            ...
            <TableBody>{runs.map((r: any) => (
              <TableRow key={r.id} className="cursor-pointer hover:bg-zinc-50" data-testid={`payroll-row-${r.id}`}>
                <TableCell className="font-medium"><Link href={`${rolePrefix}/payroll/${r.id}`} className="hover:underline">{r.users_profile?.full_name}</Link></TableCell>
