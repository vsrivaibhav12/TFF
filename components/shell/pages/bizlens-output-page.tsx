...
export default async function BizlensOutputPage({ params, rolePrefix }: Props) {
  ...
  if (isEmpty) {
    return (
      <div className="tff-card p-10 text-center">
        ...
        <Link
          href={`${rolePrefix}/bizlens/${reportId}/input`}
  ...
  const priorReports = snapshots
    .filter(s => s.data_json && typeof s.data_json === 'object')
    .map(s => computeReport({
  ...
  return (
    <div className="tff-stack-lg pb-20">
      <div className="flex items-center justify-between">
        <Link href={`${rolePrefix}/bizlens`} className="inline-flex items-center gap-1 text-sm text-zinc-500 hover:text-zinc-900">
