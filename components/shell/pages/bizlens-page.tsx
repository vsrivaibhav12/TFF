...
export default async function BizlensPage({ rolePrefix }: Props) {
  const me = await requireRole(['admin', 'team']);
  await requireCapabilityOrRedirect(me, 'bizlens.view');
              ...
              <div key={client.id} className="tff-card p-5 transition-colors hover:border-teal-300">
                ...
                {report ? (
                  <>
                    ...
                    <div className="mt-5 flex gap-2">
                      <Button variant="outline" size="sm" className="flex-1" asChild>
                        <Link href={`${rolePrefix}/bizlens/${report.id}/input`}>Edit data</Link>
                      </Button>
                      <Button size="sm" className="flex-1" asChild>
                        <Link href={`${rolePrefix}/bizlens/${report.id}/output`}>
                  ...
                  <>
                    ...
                    <div className="mt-4">
                      <CreateBizlensDialog clientId={client.id} role={role} />
                    </div>
                  </>
                )}
              </div>
