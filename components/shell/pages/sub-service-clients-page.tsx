...
import LinkClientForm from '@/components/services/sub-services/[id]/clients/link-client-form';
...
export default async function SubServiceClientsPage({ params, rolePrefix }: Props) {
  ...
  return (
    <div className="space-y-8">
      <Link href={`${rolePrefix}/services`} className="inline-flex items-center gap-1 text-sm text-zinc-500 hover:text-zinc-900">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  return (
                    <div key={link.id} className="flex items-center justify-between px-5 py-3 hover:bg-zinc-50 transition-colors">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <DockLink item={{ type: 'client', id: client.id }} href={`${rolePrefix}/clients/${client.id}`} className="text-sm font-medium text-zinc-900 hover:text-teal-700">
          ...
          <aside className="space-y-6">
            <div className="tff-card p-5 space-y-3">
              ...
              <Link href={`${rolePrefix}/tasks/bulk-create?sub_service_id=${subService.id}`} className="w-full flex">
