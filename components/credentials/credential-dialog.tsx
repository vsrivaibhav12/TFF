...
function emptyForm(initial?: any) {
  return {
    id: initial?.id,
...
export default function CredentialDialog({ clients, initial, children }: { clients: { id: string; business_name: string }[]; initial?: any; children: React.ReactNode }) {
  ...
  // Re-sync form when dialog opens for a different row
  useEffect(() => {
    if (open) {
  ...
  async function onDelete() {
    if (!form.id) return;
    const ok = await confirm({ title: 'Delete credential', description: 'Delete credential?' });
    if (!ok) return;
    startTransition(async () => {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="max-w-md">
          ...
          <DialogFooter className="gap-2">
            {initial && <Button variant="outline" onClick={onDelete} disabled={pending}><Trash2 className="h-4 w-4" /> Delete</Button>}
