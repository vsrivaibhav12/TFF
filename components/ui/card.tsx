...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'rounded-xl bg-white border border-zinc-200/50 shadow-card hover:shadow-card-hover hover:border-zinc-200 transition-colors transition-shadow duration-300',
        className
      )}
      {...props}
    />
