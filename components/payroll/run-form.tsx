...
export default function PayrollRunForm({ team }: { team: any[] }) {
  ...
  const now = new Date();
  const [f, setF] = useState({ user_id: '', year: now.getFullYear(), month: now.getMonth() + 1, total_working_days: '' as string | number });
  function run() {
    startTransition(async () => {
      const payload = {
        ...f,
        total_working_days: f.total_working_days === '' ? undefined : Number(f.total_working_days),
      };
      const r = await runPayrollAction(payload as any);
      ...
      <DialogContent>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="space-y-2"><Label>Working days</Label><Input type="number" min={1} max={31} placeholder="Auto" value={f.total_working_days} onChange={(e) => setF({ ...f, total_working_days: e.target.value ? Number(e.target.value) : '' })} /></div>
          </div>
        </div>
