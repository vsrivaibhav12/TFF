        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
           ...
           <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                 <span className={`text-3xl font-semibold ${report.monthly.opProfit >= 0 ? 'text-teal-600' : 'text-rose-600'}`}>
                    {fmtMoney(report.monthly.opProfit)}
                 </span>
                 {report.monthly.opProfit >= 0 ? <TrendingUp className="w-5 h-5 text-teal-500" /> : <TrendingDown className="w-5 h-5 text-rose-500" />}
              </div>
           </div>
