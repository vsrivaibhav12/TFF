        ...
        <Card className="lg:col-span-3 border border-zinc-200 rounded-xl overflow-hidden">
          <CardHeader className="p-8 pb-0">
            <div className="flex justify-between items-center">
              ...
              <div className={`text-xs font-bold px-3 py-1.5 rounded-full ${summary.overallHealth === 'good' ? 'bg-teal-100 text-teal-700' : summary.overallHealth === 'bad' ? 'bg-rose-100 text-rose-700' : 'bg-amber-100 text-amber-700'}`}>
                {summary.healthLabel}
              </div>
            </div>
          </CardHeader>
              ...
              {summary.keyMetrics?.map((m: any) => (
                <div key={m.label} className="p-5 rounded-xl bg-zinc-50 border border-zinc-100 space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {m.status === 'good' && <TrendingUp className="h-4 w-4 text-teal-500" />}
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
                  {summary.healthSnapshot.map((h: any) => (
                    <div key={h.indicator} className="p-4 rounded-xl bg-zinc-50 border border-zinc-100 space-y-1">
                      ...
                      <Badge className={`text-[9px] px-2 py-0 rounded-full ${h.status === 'good' ? 'bg-teal-100 text-teal-700' : h.status === 'bad' ? 'bg-rose-100 text-rose-700' : 'bg-amber-100 text-amber-700'}`}>{h.status}</Badge>
                    </div>
                  ))}
                </div>
              </div>
