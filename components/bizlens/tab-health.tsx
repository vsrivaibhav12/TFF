...
function RiskTag({ risk }: { risk: any }) {
  const c = risk.level === 'good' ? 'bg-teal-100 text-teal-700' : risk.level === 'bad' ? 'bg-rose-100 text-rose-700' : risk.level === 'neutral' ? 'bg-zinc-100 text-zinc-500' : 'bg-amber-100 text-amber-700';
...
export default function HealthTab({ report, wcc, debtFreedom }: any) {
  ...
  const overallClass = badCount >= 3 ? 'bg-rose-500' : (badCount >= 1 || medCount >= 3) ? 'bg-amber-500' : 'bg-teal-500';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <HealthCard title="Leverage (D/E)" risk={deRisk}>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="p-3 bg-zinc-50 rounded-lg"><div className="text-[9px] font-bold text-zinc-400 uppercase">Equity</div><div className="text-sm font-semibold text-teal-600">{fmtMoney(bs.eq)}</div></div>
            </div>
          </HealthCard>
          ...
          <HealthCard title="Interest Coverage" risk={icrRisk}>
            ...
            {h.isInterestFree && <p className="text-xs text-teal-600 font-medium mt-2">Zero interest obligations — all profit retained.</p>}
          </HealthCard>
        </div>
