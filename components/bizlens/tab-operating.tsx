...
export default function OperatingTab({ report, breakEven }: any) {
  ...
  const kpis = [
    ...
    { label: 'Operating Profit', value: fmtMoney(m.opProfit), sub: fmtPct(m.opPct), color: m.opProfit >= 0 ? 'text-teal-600' : 'text-rose-600', tag: fmtPct(m.opPct) },
    { label: 'Actual P&L', value: fmtMoney(t.actualPL), sub: `Op: ${fmtMoney(t.opTotal)} + Inv: ${fmtMoney(t.INV)} + OI: ${fmtMoney(t.OI)} − NC: ${fmtMoney(t.NC)}`, color: t.actualPL >= 0 ? 'text-teal-600' : 'text-rose-600' },
  ];
              ...
              <div key={k.label} className="p-4 rounded-xl bg-zinc-50 border border-zinc-100 space-y-2 relative">
                ...
                {k.mosPct != null && (
                  <div className="w-full h-1.5 bg-zinc-200 rounded-full overflow-hidden">
                    <div className={`h-full rounded-full transition-all ${(k.mosPct ?? 0) < 0.1 ? 'bg-rose-500' : 'bg-teal-500'}`}
        ...
        <CardContent className="p-8 pt-0">
          <div className="space-y-3">
            {[
              ...
              { label: 'Operating Profit', val: m.opProfit, color: m.opProfit >= 0 ? 'bg-teal-500' : 'bg-rose-500' },
            ].map((item) => {
          ...
          <CardContent className="p-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              ...
              <div className="p-6 bg-zinc-50 border border-zinc-100 rounded-xl text-center">
                ...
                <div className={`text-2xl font-semibold ${breakEven.dailyProfit >= 0 ? 'text-teal-600' : 'text-rose-600'}`}>{fmtMoney(breakEven.dailyProfit)}</div>
              </div>
            </div>
