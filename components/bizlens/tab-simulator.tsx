              ...
              {[
                ...
                <div key={s.label} className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`text-lg font-semibold ${(s.goodUp ? s.val >= 0 : s.val <= 0) ? 'text-teal-600' : 'text-rose-600'}`}>
                      {s.val > 0 ? '+' : ''}{s.val}%
                    </span>
                  </div>
            ...
            <div className="bg-zinc-50 rounded-xl p-8 border border-zinc-100 space-y-6">
              <div className="text-center space-y-2">
                ...
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span className={`text-sm font-semibold ${delta >= 0 ? 'text-teal-600' : 'text-rose-600'}`}>{delta >= 0 ? '+' : ''}{fmtMoney(delta)}</span>
                </div>
              </div>
            ...
            {[{ l: '1 Month', p: proj1 }, { l: '6 Months', p: proj6 }, { l: '12 Months', p: proj12 }].map(({ l, p }) => (
              <div key={l} className="p-5 rounded-xl bg-zinc-50 border border-zinc-100 space-y-3">
                ...
                <div className="space-y-2">
                  {[{ n: 'Sales', v: fmtMoney(p.sales) }, { n: 'Profit', v: fmtMoney(p.op), c: p.op >= 0 ? 'text-teal-600' : 'text-rose-600' }, { n: 'Safety', v: fmtMoney(p.mos) }].map(x => (
      ...
      <Card className="border border-zinc-200  rounded-xl overflow-hidden">
        ...
        <CardContent className="p-8">
          {!opt ? <p className="text-xs text-zinc-400 italic">Click Calculate to see optimum scenarios.</p> :
            opt.reason === 'Target already achieved' ? <p className="text-xs text-teal-600 font-bold">✓ Target already achieved!</p> :
