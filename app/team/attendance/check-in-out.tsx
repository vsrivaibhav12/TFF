...
export default function CheckInOut({
  ...
  function checkIn() {
    startTransition(async () => {
      ...
      if (r.success) { toast.success(geo ? `Checked in at ${geo.lat.toFixed(4)}, ${geo.lng.toFixed(4)} (±${Math.round(geo.accuracy)}m)` : 'Checked in'); router.refresh(); }
  ...
  function checkOut() {
    startTransition(async () => {
      ...
      if (r.success) { toast.success('Checked out'); router.refresh(); }
      else toast.error(r.error);
    });
  }
