...
export default async function WorkDoneReportsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const exportData = summary.map((r) => ({
  ...
  return (
    <div className="space-y-8">
      <Link href="/team" className="inline-flex items-center gap-1 text-sm text-zinc-500 hover:text-zinc-900">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        ...
        <ExportButton data={exportData} filename="workdone-reports" format="excel" />
      </div>

      <form className="flex items-center gap-3 tff-card tff-card-pad" method="get">
