...
export default async function TeamVcfoClientPage({ params }: { params: { id: string } }) {
  ...
  const chartLabels = sorted.map((s: any) => `${s.month.toString().padStart(2, '0')}/${s.year}`);
  const burnData = sorted.map((s: any) => s.monthly_burn || 0);
  const runwayData = sorted.map((s: any) => (s.cash_in_bank && s.monthly_burn > 0 ? Math.round((s.cash_in_bank / s.monthly_burn) * 10) / 10 : 0));
