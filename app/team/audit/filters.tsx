...
export default function AuditFilters({ initial }: { initial: { actor?: string; action?: string; entity?: string; from?: string; to?: string } }) {
  ...
  function apply() {
    ...
    router.push(`${prefix}/audit?${params.toString()}`);
  }
  function clear() { router.push(`${prefix}/audit`); }
  return (
    <div className="tff-card tff-card-pad grid grid-cols-1 md:grid-cols-5 gap-3">
