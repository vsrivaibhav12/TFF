    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="mt-2 flex items-center gap-2 flex-wrap">
          ...
          <Link
            href={`/team/clients/${(task as any).client_id}`}
        ...
        <aside className="space-y-6">
          <div className="tff-card p-6">
            ...
            <dl className="space-y-2 text-sm">
              ...
              <DetailItem label="Created" value={formatDateIST((task as any).created_date) || '—'} />
              <DetailItem label="Started" value={formatDateIST((task as any).started_date) || 'Not started'} />
              <DetailItem label="Completed" value={formatDateIST((task as any).completed_date) || 'Not completed'} />
              {(task as any).completed_date && (task as any).created_date && (
                <DetailItem label="Turnaround" value={`${turnaroundDays((task as any).created_date, (task as any).completed_date)} days`} />
              )}
              ...
              <DetailItem label="Service" value={(task as any).sub_services ? `${(task as any).sub_services.services?.name ?? ''} › ${(task as any).sub_services.name}` : '—'} />
              {(task as any).is_billable && (
                <DetailItem label="Billing" value={`Billable · ${(task as any).bill_reference || 'No ref'}`} />
              )}
            </dl>
          </div>
