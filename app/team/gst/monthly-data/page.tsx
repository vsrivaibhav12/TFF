...
export default async function GstMonthlyDataPage() {
  ...
  await requireCapabilityOrRedirect(me, 'compliance.view');
  ...
  const exportData = (rows ?? []).map((r: any) => {
    ...
    return {
      ...
      turnover_taxable: r.turnover_taxable ?? 0,
      output_tax: outputTax,
      itc2b,
      cash,
      carry_forward_itc: r.carry_forward_itc ?? 0,
    };
  });
    ...
    <div className="tff-stack-lg">
      <div className="tff-page-header">
        <div className="flex items-center gap-2">
          <Link href="/team/gst" className="text-sm text-zinc-500 hover:text-zinc-900"><ArrowLeft className="h-4 w-4" /></Link>
          <div>
        ...
        <ExportButton data={exportData} filename="gst-monthly-data" format="excel" />
      </div>
