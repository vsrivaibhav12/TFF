...
export default async function PortalVcfoPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {!latest ? (
        <div className="tff-card tff-card-pad bg-zinc-50 text-sm text-zinc-500">Your engagement team will publish the first vCFO snapshot here.</div>
      ) : (
        <>
...
function Stat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return <div className={`tff-card tff-card-pad ${highlight ? 'border-teal-200 bg-teal-50' : ''}`}><div className="text-xs text-zinc-500 uppercase tracking-wide">{label}</div><div className={`mt-2 text-2xl font-semibold tabular-nums ${highlight ? 'text-teal-800' : 'text-zinc-900'}`}>{value}</div></div>;
}
  ...
  return (
    <div className="tff-card tff-card-pad">
      ...
      <div className="mt-2 flex items-center gap-2">
        <span className={`text-2xl font-semibold tabular-nums ${isGood ? 'text-teal-600' : 'text-rose-600'}`}>
          {variance > 0 ? '+' : ''}{variance}%
        </span>
        {isGood ? <TrendingUp className="h-5 w-5 text-teal-500" /> : <TrendingDown className="h-5 w-5 text-rose-500" />}
      </div>
