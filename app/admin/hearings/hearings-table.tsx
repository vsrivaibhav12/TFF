...
import { updateHearingStatusAction } from '@/lib/actions/notices';
...
export default function HearingsTable({ hearings }: { hearings: Hearing[] }) {
  ...
  function toggleAll() {
    ...
    else setSelected(new Set(sorted.map((h) => h.id)));
  }
  ...
  async function bulkChangeStatus(ids: string[], status?: string) {
    ...
    await Promise.all(
      ids.map(async (id) => {
        const r = await updateHearingStatusAction({ id, status: status as any });
        if (r.success) success++; else failed++;
      })
    );
    if (failed > 0) toast.warning(`${success} updated, ${failed} failed`);
    else toast.success(`${success} hearings updated`);
    setSelected(new Set());
      ...
      <div className="tff-card overflow-hidden">
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {sorted.map((h) => (
                <TableRow key={h.id} className={selected.has(h.id) ? 'bg-teal-50/40' : ''} data-testid={`hearing-row-${h.id}`}>
                  <TableCell>
                    <Checkbox checked={selected.has(h.id)} onCheckedChange={() => toggle(h.id)} aria-label={`Select hearing ${h.subject}`} />
                  </TableCell>
