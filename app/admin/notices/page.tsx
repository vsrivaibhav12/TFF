...
import NoticeDialog from '@/app/team/notices/notice-dialog';
...
function buildNoticeUrl(sp: Record<string, string | undefined>, overrides: Record<string, string | undefined>) {
  ...
  return qs ? `/admin/notices?${qs}` : '/admin/notices';
}
...
export default async function AdminNoticesPage({ searchParams }: { searchParams: { status?: string; type?: string; client?: string; due_from?: string; due_to?: string } }) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Notices"
        subtitle="Tax department correspondence across all clients."
        actions={
          <>
      ...
      <div className="space-y-3">
        ...
        />
        <SavedViewsBar scope="admin.notices" views={views as any} />
      </div>
