...
export default function ServiceDialog({ categories, initial, children }: { categories: any[]; initial?: any; children: React.ReactNode }) {
  ...
  function save() {
    ...
    startTransition(async () => {
      const r = await upsertServiceAction({ ...f, category_id: f.category_id || undefined, service_kind: f.service_kind || undefined });
  ...
  async function remove() {
    if (!f.id) return;
    const ok = await confirm({ title: 'Delete Service', description: 'Delete this service? Existing client assignments will be preserved.' });
    if (!ok) return;
    startTransition(async () => {
          ...
          <div className="space-y-3">
            <div className="space-y-2"><Label>Category</Label>
              <Select value={f.category_id || '__none__'} onValueChange={(v) => set('category_id', v === '__none__' ? '' : v)}>
                <SelectTrigger data-testid="svc-category"><SelectValue placeholder="Uncategorized" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="__none__">Uncategorized</SelectItem>
                  {categories.map((c) => <SelectItem key={c.id} value={c.id}>{c.name}</SelectItem>)}
                </SelectContent>
              </Select>
              <p className="text-xs text-zinc-500">Optional. Group services visually on this page.</p>
            </div>
