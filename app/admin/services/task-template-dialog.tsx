...
export default function TaskTemplateDialog({ subServiceId, subServiceName, initial, children }: Props) {
  ...
  function set(k: keyof typeof f, v: any) { setF((p) => ({ ...p, [k]: v })); }

  function save() {
    if (!f.title.trim()) { toast.error('Title is required'); return; }
    startTransition(async () => {
      const r = await upsertTaskTemplateAction(f);
  ...
  async function remove() {
    if (!f.id) return;
    const ok = await confirm({ title: 'Delete Template', description: 'Delete this task template?' });
    if (!ok) return;
    startTransition(async () => {
