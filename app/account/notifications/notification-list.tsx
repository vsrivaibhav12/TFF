...
const TYPE_META: Record<string, { icon: React.ReactNode; color: string; bg: string }> = {
  ...
  task_completed: { icon: <Check className="h-4 w-4" />, color: 'text-teal-600', bg: 'bg-teal-50 border-teal-100' },
...
export default function NotificationList({ items: initial }: { items: NotificationItem[] }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <div className="tff-card tff-card-pad bg-zinc-50 text-sm text-zinc-500 text-center">
          {filter === 'unread' ? 'No unread notifications.' : 'Nothing yet. Activity from your tasks, queries, and compliance will appear here.'}
        </div>
      ) : (
