...
export async function updateSession(request: NextRequest) {
  ...
  // 2) Logged in but hitting /login → redirect straight to their dashboard
  if (path === '/login' && user) {
  ...
  if (matchedProtected && user) {
    ...
    if (!isNeutral) {
      // H-11: Try JWT user_metadata first to avoid a second DB call.
      // The role is set in user_metadata during user creation (team.ts, client-service.ts).
      let role = user.user_metadata?.role as string | undefined;
      let isActive = true;

      // Fallback to DB if JWT metadata is missing (legacy users or metadata not set)
      if (!role) {
        const { data: profile } = await supabase
          .from('users_profile')
          .select('role, is_active')
        ...
        // Inactive or no profile → bounce to /login
