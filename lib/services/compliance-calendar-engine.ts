...
// =============================================================================
// Service-driven compliance events (new)
// =============================================================================

/**
 * For each active client_sub_services row, generate virtual compliance events
 * based on the parent sub_service's frequency and due date configuration.
 * These are stored in compliance_calendar_events with rule_id=NULL so they
 * don't collide with rule-driven events.
 */
export async function refreshServiceDrivenComplianceEvents(opts: {
  fromIso?: string;
  toIso?: string;
  clientId?: string;
  ...
  let q = sb
    ...
    .select('id, client_id, sub_services(id, code, name, frequency, due_day_of_month, due_day_of_quarter, due_month, service_id)')
    .eq('is_active', true);
  if (opts.clientId) q = q.eq('client_id', opts.clientId);
  const { data: cssRows, error: cssErr } = await q;
  if (cssErr) throw cssErr;
  if (!cssRows || cssRows.length === 0) return { generated: 0, clientSubServices: 0 };
  ...
  for (const row of cssRows as any[]) {
    const ss = row.sub_services;
    if (!ss) continue;
    const events = generateSubServiceEvents(ss, row.client_id, fromIso, toIso);
    for (const e of events) {
      allEvents.push({ ...e, status: 'upcoming' as const });
    }
  }
...
function generateSubServiceEvents(
  subService: {
    id: string;
    code: string;
    name: string;
  ...
  fromIso: string,
  toIso: string,
): Array<{ client_id: string; rule_id: null; sub_service_id: string; rule_code: string; period_label: string; due_date: string }> {
  const events: Array<{ client_id: string; rule_id: null; sub_service_id: string; rule_code: string; period_label: string; due_date: string }> = [];
  const fromY = parseInt(fromIso.slice(0, 4), 10);
  const fromM = parseInt(fromIso.slice(5, 7), 10);
  const toY = parseInt(toIso.slice(0, 4), 10);
  const toM = parseInt(toIso.slice(5, 7), 10);

  const code = subService.code || 'SRV';

  if (subService.frequency === 'monthly') {
    let y = fromY, m = fromM;
    while (y < toY || (y === toY && m <= toM)) {
      const dueDay = Math.min(subService.due_day_of_month ?? 1, lastDayOfMonth(y, m));
      events.push({
        client_id: clientId,
        rule_id: null,
        sub_service_id: subService.id,
        rule_code: code,
        period_label: `${monthName(m)} ${y}`,
        due_date: dateISO(y, m, dueDay),
      });
      m += 1;
      if (m > 12) { m = 1; y += 1; }
    }
  ...
  if (subService.frequency === 'quarterly') {
    // Calendar quarters for simplicity; align to FY later if needed
    const yearsToCheck = new Set<number>();
    yearsToCheck.add(fromY); yearsToCheck.add(toY); yearsToCheck.add(fromY - 1); yearsToCheck.add(toY + 1);
    for (const y of yearsToCheck) {
      for (let q = 1; q <= 4; q++) {
        const qLastM = q * 3;
        const dueMAbs = qLastM + 1; // default: month after quarter end
        const dueY = y + Math.floor((dueMAbs - 1) / 12);
        const dueM = ((dueMAbs - 1) % 12) + 1;
        const dueDay = Math.min(subService.due_day_of_month ?? lastDayOfMonth(dueY, dueM), lastDayOfMonth(dueY, dueM));
        events.push({
          client_id: clientId,
          rule_id: null,
          sub_service_id: subService.id,
          rule_code: code,
          period_label: `Q${q} ${y}`,
          due_date: dateISO(dueY, dueM, dueDay),
        });
      }
    }
    return events;
  ...
  if (subService.frequency === 'annually') {
    const yearsToCheck = new Set<number>();
    yearsToCheck.add(fromY); yearsToCheck.add(toY); yearsToCheck.add(fromY - 1); yearsToCheck.add(toY + 1);
    for (const y of yearsToCheck) {
      const dueM = subService.due_month ?? 7;
      const dueDay = Math.min(subService.due_day_of_month ?? lastDayOfMonth(y, dueM), lastDayOfMonth(y, dueM));
      events.push({
        client_id: clientId,
        rule_id: null,
        sub_service_id: subService.id,
        rule_code: code,
        period_label: `FY ${y}-${(y + 1).toString().slice(2)}`,
        due_date: dateISO(y, dueM, dueDay),
      });
    }
    return events;
  }

  // on_demand: no recurring events
  return events;
}
