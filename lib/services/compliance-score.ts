...
export async function computeComplianceScore(clientId: string): Promise<ComplianceBreakdown> {
  ...
  const tdsFiled = recentTds.filter((f) => f.status === 'filed').length;
  const tdsTotal = recentTds.length || 1;

  // IT: last 2 FYs
  const currentFY = now.getMonth() >= 3 ? now.getFullYear() : now.getFullYear() - 1;
  const recentIt = it.filter((f) => f.fy_ending_year >= currentFY - 1);
  const itFiled = recentIt.filter((f) => f.status === 'filed').length;
