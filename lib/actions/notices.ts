...
export async function upsertNoticeAction(input: z.infer<typeof noticeSchema>): Promise<ActionResult<{ id: string }>> {
  try {
    ...
    if (id) {
      ...
      await writeAudit({ action: 'notice.update', entity_type: 'notice', entity_id: id, performed_by: me.id, details: { client_id: rest.client_id, status: rest.status } });
    ...
    if (error) return fail(error.message, 'DB');
    await writeAudit({ action: 'notice.create', entity_type: 'notice', entity_id: data.id, performed_by: me.id, details: { client_id: rest.client_id, notice_type: rest.notice_type } });
    revalidatePath(`/team/clients/${rest.client_id}`);
    revalidatePath('/portal/notices');
...
export async function upsertHearingAction(input: z.infer<typeof hearingSchema>): Promise<ActionResult<{ id: string }>> {
  try {
    ...
    if (id) {
      ...
      if (error) return fail(error.message, 'DB');
      await writeAudit({ action: 'hearing.update', entity_type: 'hearing', entity_id: id, performed_by: me.id });
      revalidatePath('/team/hearings');
      return ok({ id });
    }
    const { data, error } = await sb.from('hearings').insert(rest).select('id').single();
    if (error) return fail(error.message, 'DB');
    await writeAudit({ action: 'hearing.create', entity_type: 'hearing', entity_id: data.id, performed_by: me.id, details: { client_id: rest.client_id } });
    revalidatePath('/team/hearings');
    return ok({ id: data.id });
  } catch (e: any) {
    return fail(e?.message ?? 'unknown', e?.code ?? 'UNKNOWN');
...
export async function updateNoticeStatusAction(input: z.infer<typeof updateStatusSchema>): Promise<ActionResult<void>> {
  try {
    ...
    if (error) return fail(error.message, 'DB');
    await writeAudit({ action: 'notice.status_change', entity_type: 'notice', entity_id: parsed.data.id, performed_by: me.id, details: { status: parsed.data.status } });
...
export async function updateHearingStatusAction(input: z.infer<typeof updateHearingStatusSchema>): Promise<ActionResult<void>> {
  try {
    ...
    if (error) return fail(error.message, 'DB');
    await writeAudit({ action: 'hearing.status_change', entity_type: 'hearing', entity_id: parsed.data.id, performed_by: me.id, details: { status: parsed.data.status } });
