...
export async function upsertTaskTemplateAction(input: z.infer<typeof taskTemplateSchema>): Promise<ActionResult<{ id: string }>> {
  try {
    ...
    const { data, error } = await sb.from('task_templates').upsert(payload, { onConflict: 'id' }).select('id').single();
...
export async function upsertTaskTemplateStepAction(input: z.infer<typeof stepSchema>): Promise<ActionResult<{ id: string }>> {
  try {
    ...
    if (!parsed.data.id) {
      delete payload.id;
      payload.created_at = new Date().toISOString();
      // Auto-assign step_order to avoid unique-constraint collision on (task_template_id, step_order)
      const { data: maxRow } = await sb
        .from('task_template_steps')
        .select('step_order')
        .eq('task_template_id', parsed.data.task_template_id)
        .eq('is_deleted', false)
        .order('step_order', { ascending: false })
        .limit(1)
        .maybeSingle();
      payload.step_order = (maxRow?.step_order ?? 0) + 1;
    }
...
export async function reorderTaskTemplateStepsAction({
  ...
  try {
    ...
    // Pass 1: shift all reordered steps to negative temporary values
    for (let i = 0; i < ids_in_order.length; i++) {
      await sb.from('task_template_steps')
        .update({ step_order: -(i + 1) })
        .eq('id', ids_in_order[i])
        .eq('task_template_id', task_template_id);
    }
    // Pass 2: set final positive values
    for (let i = 0; i < ids_in_order.length; i++) {
      await sb.from('task_template_steps')
        .update({ step_order: i + 1 })
        .eq('id', ids_in_order[i])
