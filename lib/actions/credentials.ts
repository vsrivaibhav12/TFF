...
export async function upsertCredentialAction(input: CredentialInput): Promise<ActionResult<{ id: string }>> {
  try {
    ...
    const encFields: any = {};
    if (password) encFields.encrypted_password = encryptString(password);
    if (security_answer) encFields.encrypted_security_answer = encryptString(security_answer);

    if (id) {
      // Update path: prevent client_id mutation — vault records must stay bound to their client.
      const { client_id: _drop, ...updateFields } = rest;
    ...
    // Insert path: client_id must be included.
...
export async function revealCredentialAction(id: string): Promise<ActionResult<{ password: string; security_answer: string | null }>> {
  try {
    ...
    const { data, error } = await sb
      .from('credentials')
      .select('id, client_id, portal_name, encrypted_password, encrypted_security_answer, is_deleted')
      .eq('id', id)
      .maybeSingle();
    ...
    await sb.from('credentials').update({ last_used_date: todayIST() }).eq('id', id);
    await writeAudit({
