...
async function main() {
  ...
  const { data: sess, error: se } = await sbAnon.auth.signInWithPassword({ email: 'info@fiscalfulcrum.in', password: '__ADMIN_SEED_PASSWORD__' });
  console.log(`\nAdmin login: ${se ? 'FAIL: '+se.message : 'OK token='+sess.session?.access_token.slice(0,20)+'...'}`);
  // try team login
  const { data: ts, error: te } = await sbAnon.auth.signInWithPassword({ email: 'team.demo@fiscalfulcrum.in', password: '__TEAM_SEED_PASSWORD__' });
